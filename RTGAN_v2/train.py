...
if __name__ == '__main__':
    ...
    train_set = TrainDatasetFromFolder('/dockerx/')
    val_set = ValDatasetFromFolder('/dockerx/')
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        with torch.no_grad():
            ...
            for val_off, val_on in val_bar:
                ...
                batch_mse = mse_loss(rt, on)
                valing_results['mse'] += batch_mse * batch_size
                batch_ssim = calc_ssim(rt, on).item()
                valing_results['ssims'] += batch_ssim * batch_size
                valing_results['psnr'] = 10 * log10((on.max()**2) / (valing_results['mse'] / valing_results['batch_sizes']))
...
#try ToTensor() if this does not work (display_transform)
                val_images.extend(
                    [off.data.cpu().squeeze(0), on.data.cpu().squeeze(0),
                     rt.data.cpu().squeeze(0)])
...
"""
        ...
        f = open('statistics/' + 'RTGAN_' + str(datetime.today()) + '_train_results.txt', 'a+')
