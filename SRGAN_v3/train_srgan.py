...
if __name__ == '__main__':
    ...
    if os.path.exists(netG_ckp_path):
        ...
        if 'srresnet_mse' in netG_ckp_path:
            print('SRResNet-MSE model restore success! Start training in SRGAN with ', MSE_OR_VGG, ' loss!')  
        elif 'srgan' in netG_ckp_path: 
            print('SRGAN model restore success! Continue traning in SRGAN with ', MSE_OR_VGG, ' loss!')
        else: 
            print('Double check the loaded checkpoint, the current one is neither SRResNet-MSE nor SRGAN!')
    else: 
         start_epoch = 1 
...
# SRGAN training     
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        if epoch % 1 == 0 and epoch != 0:
            save_ckp(checkpoint_netD, 'netD', UPSCALE_FACTOR, epoch, 'srgan', MSE_OR_VGG)
            save_ckp(checkpoint_netG, 'netG', UPSCALE_FACTOR, epoch, 'srgan', MSE_OR_VGG)
